...
from VisualizeNetwork import create_network_evolution_gif, visualizeNetwork
...
def plot_evolution_comparison(
    ...
    axes[0].plot(time_drug, total_cancer_drug, 'r-', linewidth=2, label='WITH drug')
    axes[0].plot(time_nodrug, total_cancer_nodrug, 'r--', linewidth=2, label='NO drug')
    ...
    axes[1].plot(time_drug, total_tumor_drug, 'b-', linewidth=2, label='WITH drug')
    axes[1].plot(time_nodrug, total_tumor_nodrug, 'b--', linewidth=2, label='NO drug')
    ...
    axes[2].set_title('Drug Concentration', fontsize=12, fontweight='bold')
...
def visualize_sample(
    ...
    try:
        ...
        # Save initial and final frames as static images using visualizeNetwork
        try:
            x_init = X_drug[:, 0].detach().cpu().numpy().reshape(-1, 1)
            x_final = X_drug[:, -1].detach().cpu().numpy().reshape(-1, 1)
            _, _, init_path = visualizeNetwork(x_init, p_fixed, title_prefix=f"{sample_id}_with_drug_init_", output_dir=viz_with, dpi=120, visualize=False, save=True)
            _, _, final_path = visualizeNetwork(x_final, p_fixed, title_prefix=f"{sample_id}_with_drug_final_", output_dir=viz_with, dpi=120, visualize=False, save=True)
            print(f"  [SUCCESS] WITH-DRUG images: init={init_path}, final={final_path}")
        except Exception as e:
            print(f"  [FAILED] Saving WITH-DRUG images: {e}")
    except Exception as e:
    ...
    try:
        ...
        # Save initial and final frames as static images using visualizeNetwork
        try:
            x_init_n = X_nodrug[:, 0].detach().cpu().numpy().reshape(-1, 1)
            x_final_n = X_nodrug[:, -1].detach().cpu().numpy().reshape(-1, 1)
            _, _, init_path_n = visualizeNetwork(x_init_n, p_fixed, title_prefix=f"{sample_id}_no_drug_init_", output_dir=viz_without, dpi=120, visualize=False, save=True)
            _, _, final_path_n = visualizeNetwork(x_final_n, p_fixed, title_prefix=f"{sample_id}_no_drug_final_", output_dir=viz_without, dpi=120, visualize=False, save=True)
            print(f"  [SUCCESS] NO-DRUG images: init={init_path_n}, final={final_path_n}")
        except Exception as e:
            print(f"  [FAILED] Saving NO-DRUG images: {e}")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    clinical_data_path = "data/nature_immune_processed"
    slide_response_path = "data/nature_immune_processed/slide_responses.csv"
    output_base = "sample_visualizations_static"
